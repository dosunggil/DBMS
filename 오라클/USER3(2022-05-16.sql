...
/*
    ...
    PK 를 설정하기 위해 SC_SEQ 라는 칼럼이 추가된 상태
    
    SC_SEQ 칼럼이 없이 PK를 설정하려면 
    학번 + 과목명 으로 두개의 칼럼을 묶어서
    PK 로 해야한다.
    다수의 칼럼을 묶음으로 PK를 설정하는 것을 
    Super Key 라고 한다.
    ...
    WHRER sc_num = "1111" AND sc_subject = "국어"
    
    DELETE FROM tbl_student 
    WHRER sc_num = "1111" AND sc_subject = "국어"
*/

-- 학번 S0044 학생의 미술점수를 50점에서 80점으로 바꾸고 싶다.
...
-- 성적 조회를 하였는데 학번과 점수만 나타나서 누구인지(학생이름)를 모르겠다.
-- 두 개 이상의 table 을 서로 연동하여 데이터를 확인하기
-- JOIN 하여 데이터 검색
-- 1. tbl_score table 데이터를 전부 펼치고
-- 2. sc_stnum 칼럼과 sc_score 칼럼 데이터를 보여주면서
-- 3. tbl_score.sc_stnum  데이터를 가지고 tbl_student.st_num 을 select 한 후
