...
CREAT(새로운 객체 생성),DROP(객체 제거), ALTER(객체의 구조 변경)
...
-- projection : 컬럼을 원하는 것만, 원하는 순서로 보기
...
-- 3. update 후 데이터 검증
-- tbl_score table 의 과목명과 tbl_subject table 의 과목명이 일치하지 않아
-- update 가 되지 않은 데이터가 있는지 확인.
select *
from tbl_score
where sc_sbcode = '001';

-- 칼럼을 처음 추가한 상태에서는 null 값이 담겨있으므로
-- 칼럼의 값이 null 인 데이터만 조회한다.
-- 결과는 없어야 한다.
select *
from tbl_score
...
-- NOT NULL 제약 조건 : 값을 INSERT 할 때 이 칼럼은 비워놓으면 안된다.
ALTER TABLE tbl_score MODIFY (sc_sbcode NOT NULL);
-- 이후 부터 데이터를 INSERT 할 때 sc_sbcode 칼럼의 데이터가 없으면 INSERT 가 거부된다.
...
*/
select  sc_stnum as 학번, sc_sbcode as 과목코드, sb_name 과목명, sc_score 점수
...
select * from tbl_score where sc_sbcode = 'SB001';
