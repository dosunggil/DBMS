...
-- MySQL 에서는 DB 와 관련된 명령을 실행할 때
-- 가장 먼저 어떤 DB를 사용할 것인지 알려줘야한다.

use mydb;
drop table tbl_test;

-- Table 을 생성할 때 칼럼의 이름은 
-- 사전에 있는 키워드는 가급적 사용하지 않는 것이 좋다.
-- 사전에 있는 키워드를 사용할 때는 접두사를 붙여 사용하자.
create table tbl_test (
	id bigint primary key auto_increment,
	t_name varchar(30)
);
/*
	DBMS 중에 가장 많이 사용하는 oracle 과 함께 
    MySQL을 사용하는데 
...
/*
    ...
    데이터를 insert 할때마다 자동으로 Sequence 값이 생성되어 추가된다. 
    5.x MySQL 에서는 AUTO_increment 칼럼에 이슈가 있었다.
    5.x 숫자 type 은 주로 int 형을 사용하는데 
    int 형은 정수 11자리이다.
    때문에 정수형 11자리 범위를 벗어나는 경우 데이터 insert 가 불가능해지는
    문제가 있었다. 
    5.7 버전 이후에는 bigint 형 type 도입되어 문제를 해결하였다.
    DB table 을 만들때 AUTO_increment 칼럼에는 반드시 BIGINT 형d을 지정한다.
    
    MySQL 의 숫자형 데이터 type
    TINYINT : 1byte : -128 ~ 127 까지만 저장 가능.
    SMALLINT : 2byte : -32,768 ~ 32,767 까지만 저장 가능.
    MEDIUMINT : 3byte : 0 ~ 16,777,215, -8,388,608 ~ 8,388,607 까지 저장 가능.
...
/*
    ...
    pk가 아닌 경우는 반드시 not null 과 unique 를 같이 설정한다.
    pk 또는 not null adn unique 설정이 되지 않았을 경우
    table 생성에서 오류가 난다.
*/
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0111','xasd','11',2,'21','123');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0133','조도성','수학과',4,'100010010','광주');
